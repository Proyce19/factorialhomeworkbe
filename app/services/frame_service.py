...
def create_frame(data):
    ...
    if not name:
        return jsonify({"message": "Please provide name for the frame"}), 400

    if not frame_finish_id:
        return jsonify({"message": "Please provide frame finish"}), 400

    if not frame_type_id:
        return jsonify({"message": "Please provide frame type"}), 400

    frame_finish = get_ff_by_id(frame_finish_id)
    if not frame_finish:
        return jsonify({"message": "The provided frame finish does not exits"}), 400

    frame_type = get_ft_by_id(frame_type_id)
    if not frame_type:
        return jsonify({"message": "The provided frame type does not exits"}), 400


    existing_frame = get_frame_by_type_and_finish(frame_type, frame_finish)

    if existing_frame:
        return jsonify({"message": "The frame with the provided type and finish exist"}), 400
...
def update_frame(id, data):
    ...
    if not existing_frame:
        return jsonify({"message": "Please provide existing frame"}), 400
    ...
    if existing_frame.frame_finish != frame_finish or existing_frame.frame_type != frame_type:
        ...
        if existing_frame_with_type_and_finish:
            return jsonify({"message": "The Frame with the provided type and finish already exists"}), 400
